...
sudo -u kpihx scp -o StrictHostKeyChecking=no "${TMP_KEYS}" kpihx-pve:/tmp/sshkeys_${VMID}.tmp
sudo -u kpihx ssh -o StrictHostKeyChecking=no kpihx-pve "sudo qm set ${VMID} --sshkeys /tmp/sshkeys_${VMID}.tmp && rm -f /tmp/sshkeys_${VMID}.tmp"
